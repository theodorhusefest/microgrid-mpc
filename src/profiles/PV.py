...
class Photovoltaic:
    def __init__(self, Pm, Im, Vm, Voc, Isc, Ns, A, Ki, Eg):
        self.V_value = 0
        self.I_value = 0
    ...
    def estimate(self):
        ...
        Irs = self.Isc/(np.exp((self.q * self.Voc)/(self.Ns * self.K * self.A * self.Tref)) - 1)
        Is = Irs
        Iph = self.Isc

        g = I - (Iph - Is * (np.exp(self.q * (V + I * Rs)/(self.Ns * self.K * self.A * self.Tref)) - 1) - (V + I * Rs)/Rsh)
    ...
    def create_nlp(self):
        ...
        Irs = self.Isc / (np.exp((self.q * self.Voc) / (self.Ns * self.K * self.A * T)) - 1)
        Is = Irs * power(T/self.Tref, 3) * np.exp((self.q * self.Eg)/(self.A * self.K) * (1/self.Tref - 1/T))
        Iph = G/self.Gref * (self.Isc + self.Ki * (T-self.Tref))

        g = I - (Iph - Is*(np.exp(self.q * (V + I * self.rs)/(self.Ns * self.K * self.A * T)) - 1)) #- (V + I * self.rs)/self.rsh)
        f = -I * V
