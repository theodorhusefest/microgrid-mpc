...
def open_loop_optimization(
    ...
    opts = {"verbose": False, "verbose_init": False, "ipopt": {"print_level": 0}, "print_time": False}
    ...
    if plot == True:
        ...
        plt.figure()
        plt.clf()
        plt.plot(linspace(0., T, N), w_opt[1::3],'-')
        plt.plot(linspace(0., T, N), w_opt[2::3],'-')
        ...
        plt.figure()
        plt.plot(linspace(0., T, N+1), w_opt[0::3])
        plt.xlabel('Time [h]')
        plt.ylabel('SOC [%]')
    ...
    return  w_opt[0::3], [w_opt[1::3], w_opt[2::3]] 